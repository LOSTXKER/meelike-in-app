...
export function generateMockBills(): void {
  ...
  const services = [
    { id: 1, name: 'Facebook ถูกใจโพสต์ (คนไทย)', unitPrice: 0.10, category: 'facebook' },
    { id: 2, name: 'Instagram Followers (คนไทย)', unitPrice: 0.15, category: 'instagram' },
    { id: 3, name: 'TikTok วิว (Fast)', unitPrice: 0.05, category: 'tiktok' },
    { id: 4, name: 'YouTube วิว (คนไทย)', unitPrice: 0.20, category: 'youtube' },
    { id: 5, name: 'Twitter Followers', unitPrice: 0.12, category: 'twitter' },
  ];
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const bill: Bill = {
      ...
      status,
      source: Math.random() > 0.3 ? 'manual' : 'store',
      customerNote: Math.random() > 0.5 ? 'ลูกค้าต้องการเร็ว' : undefined,
